...
from icd10.core.exceptions import ValidationError
...
def validate(file_obj: TemporaryUploadedFile):
    ...
    try:
        ...
        raise ValidationError("Please use utf-8 encoding")
    except:
        raise
    
    diff = list(np.setdiff1d(COLUMNS, list(df.columns)))
    if diff:
        raise ValidationError(f"Columns {', '.join(diff)} do not exist in the dataset")
    
    if file_obj.size > MAX_SIZE:
        raise ValidationError(
            f"File size ({file_obj.size/(1024*1024)} MB) exceeded "\
    ...
    if non_object_cols:
        raise ValidationError(f"Columns {', '.join(non_object_cols)} are not strings")
